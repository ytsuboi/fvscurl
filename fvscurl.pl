...
# This code is based on s3curl.pl (version: 2010-11-16) by "Amazon.com, Inc. or its affiliates."
# You can find original code at http://aws.amazon.com/code/128
#
# ChangeLog
# 2012-01-27 Yoshihiro TSUBOI <ytsuboi-at-gmail.com>
#		* Modified: To support IIJ GIO Storage Service FV/S HTTP headers
#		* Modified: Usage, Warning messages for FV/S
#		* Modified: Variable names
#		* Solved: LOCALE issue. On my Japanese env, POSIX::strftime outputed Kanji.
...
my $usage = <<USAGE;
Usage $0 --id friendly-name (or FV/S AccessKeyId) [options] -- [curl-options] [URL]
 options:
  --key SecretAccessKey       id/key are FV/S AcessKeyId and Secret (unsafe)
  --contentType text/plain    set content-type header
  --acl public-read           use a 'canned' ACL (x-iijgio-acl header)
  --contentMd5 content_md5    add x-iijgio-content-md5 header
 ...
 common curl options:
  -H 'x-iijgio-acl: public-read' another way of using canned ACLs
  -v                          verbose logging
USAGE
...
if ($cmdLineSecretKey) {
    ...
    my $keyinfo = $fvsSecretAccessKeys{$keyId};
...
my %xfvsHeaders;
$xfvsHeaders{'x-iijgio-acl'}=$acl if (defined $acl);
$xfvsHeaders{'x-iijgio-copy-source'}=$copySourceObject if (defined $copySourceObject);
$xfvsHeaders{'x-iijgio-copy-source-range'}="bytes=$copySourceRange" if (defined $copySourceRange);
...
for (my $i=0; $i<@ARGV; $i++) {
	...
	my $header = $ARGV[++$i];
        #check for host: and x-iijgio*
        if ($header =~ /^[Hh][Oo][Ss][Tt]:(.+)$/) {
            $host = $1;
        }
        elsif ($header =~ /^([Xx]-[Ii][Ii][Jj][Gg][Ii][Oo]-.+): *(.+)$/) {
            my $name = lc $1;
            my $value = $2;
            # merge with existing values
            if (exists $xfvsHeaders{$name}) {
                $value = $xfvsHeaders{$name} . "," . $value;
            }
            $xfvsHeaders{$name} = $value;
        }
    }
}
...
my $xfvsHeadersToSign = "";
foreach (sort (keys %xfvsHeaders)) {
    my $headerValue = $xfvsHeaders{$_};
    $xfvsHeadersToSign .= "$_:$headerValue\n";
}

setlocale(LC_TIME, "C");
my $httpDate = POSIX::strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime );
my $stringToSign = "$method\n$contentMD5\n$contentType\n$httpDate\n$xfvsHeadersToSign$resource";
...
push @args, ("-H", "Authorization: IIJGIO $keyId:$signature");
push @args, ("-H", "x-iijgio-acl: $acl") if (defined $acl);
push @args, ("-L");
...
if (defined $createBucket) {
    ...
    push @args, ("-X", "PUT");
    push @args, ("-H", "x-iijgio-copy-source: $copySourceObject");
} elsif (defined $postBody) {
...
WARNING: It isn't safe to put your FV/S secret access key on the
command line!  The recommended key management system is to store
your FV/S secret access keys in a file owned by, and only readable
by you.
